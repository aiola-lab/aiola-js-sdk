...
describe("AiolaStreamingClient", () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(client, "startMicStreaming").mockResolvedValue(undefined);
    });
    ...
    describe("startStreaming", () => {
        it("should initialize socket connection and audio streaming", async () => {
            ...
            expect(require("socket.io-client").io).toHaveBeenCalledWith(expect.stringContaining(mockConfig.baseUrl), expect.objectContaining({
                path: "/api/voice-streaming/socket.io",
                extraHeaders: { Authorization: mockConfig.bearer },
            }));
            // Verify startMicStreaming was called
            expect(client.startMicStreaming).toHaveBeenCalledWith(mockConfig.micConfig);
        });
    });
